# Прототип микросервиса расчета координат Космической Системы
...
Также прошу обратить внимание что изначально я расценивал вот это требования:
```На вход подаются данные о КА в формате TLE (Two-Line Element) или JSON с параметрами орбиты```. Как реализацию или одного или друго. Но ближе к концу работы решил реализовать оба вариант, поэтому центарльный сервис OrbitCalculationService выглядит перегруженным и по хорошему нуждается в рефакторинге. 

Но времени больше нет, поэтому прошу оценить работу так как она вышла.
